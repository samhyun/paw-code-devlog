...
프로젝트이다. 후보별 주석과 근거, 판정 상태를 연결해 사용자가 분석을 이어가도록
돕는다.
...
관련 논문도 찾아야 했다.
→ [문제와 맥락](docs/01-problem-and-context.md)
...
사람이 하던 판단은 그대로 둔다. PawCode가 맡은 것은 반복 실행과 정보 연결이다.

목표를 말하면 필요한 선행 단계까지 순서를 세워 실행하고, 중단돼도 이어서 돌린다.
후보마다 어떤 근거에 걸렸는지를 이름으로 보이고 관련 논문과 외부 데이터베이스를
찾아 붙인다. 무엇을 원인으로 볼지는 사람이 정한다.
...
정렬 7종과 검출기 2종을 같은 조건에서 재고 골랐다. 선택 기준과 비교 결과를
기록했고, 개 데이터에서는 정확도를 판정할 수 없어 대조 경로도 남겼다.
→ [파이프라인 설계](docs/02-pipeline-design.md)

### 문진 기반 질환 분석

시료마다 다른 질환 정보를 반영할 수 있도록 경로 가중치와 유전자 패널의 하드코딩을
제거했다. 임상 정보를 받을 수 있게 된 뒤에는 문진을 입력으로 받는 질환 갈래를
추가했다. 분석 구조는 질환과 무관하게 둔다.
→ [후보를 좁히는 방법](docs/03-candidate-review.md)
...
첫 측정이 한쪽에 유리한 조건이어서 질의를 새로 만들었다. 색인을 30배로 키운 뒤
적중률이 낮아져 임베딩 모델과 검색어 생성 방식을 다시 정했다.
...
| 문서 | 핵심 내용 |
|---|---|
| [문제와 맥락](docs/01-problem-and-context.md) | 분석 시간과 후보 검토의 어려움 |
| [파이프라인 설계](docs/02-pipeline-design.md) | 도구 비교 · 실행·재개 · 산출물 관리 |
| [후보를 좁히는 방법](docs/03-candidate-review.md) | 질환별 정렬 · 근거 계열 · 변이 판정 |
...
| [판단 기준](docs/07-decisions-and-lessons.md) | 측정 조건 · 근거 해석 · 집계와 상태 표시 |
| [확장 방향](docs/08-next-steps.md) | 데이터 범위 · 제품 기능 · 운영과 평가 |

---

## 공개 범위

이 저장소가 무엇을 공개하고 무엇을 공개하지 않는지는 [DISCLOSURE.md](DISCLOSURE.md)에 있다.
여기 나오는 후보는 검토 대상이지 원인 변이가 아니다. 진단에 쓸 수 없다.
