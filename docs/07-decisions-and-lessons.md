...
> 핵심: 측정 조건 · 근거 해석 · 집계와 상태 표시

4주 동안 구현과 검토에 사용한 기준을 정리했다.
...
작은 규모에서 나온 수치는 색인 규모가 달라지면 그대로 이어지지 않았다. 규모가 바뀌면
다시 재야 한다.

## 집계 단위

리드·변이·행은 서로 다른 것을 센다. 하나의 막대로 이으면 같은 단위처럼 보인다.
...
"임상 정보는 오지 않는다"는 답을 받고 파이프라인은 그 전제로 바꿨는데, 리포트
문구만 옛것으로 남아 있었다. "다음 단계의 핵심 입력은 임상 정보입니다"라는
문장과 요청 항목표가 그대로 있었다.

코드를 바꾼 뒤에는 코드가 만드는 문서도 함께 확인했다. 계산하지 않은 값을
"0건"으로 적거나 조회하지 못한 것과 조회 결과가 없는 것을 같게 적은 부분을 고쳤다.
