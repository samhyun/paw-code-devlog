...
| 출력 처리 경로 | 공통 종료 함수에서 대화 답변 처리 · 경로별로 붙일 내용을 한 표에서 관리 | 경로 6개 |
| 도구 권한 | 역할별 도구 제한 · 승인 토큰을 모델에게 주지 않음 | 열람자 45 / 관리자 48 |
| 요청 검증 | 실행 전 사람 확인 · ID 형식·경로 탈출 차단 · 감사 로그 · 로그인 잠금 | 토큰 TTL 30분·1회용 / 5회 실패 시 15분 잠금 |
...
| 완화됨 | 16건 (55.2%) |
| 완화된 문장 중 조사 오류 발생 | 7건 ("원인 후보으로" 등) |
| 조사 오류 없이 완화 | 9건 (31.0%) |
...
이 수치는 정규식이 문장을 바꿨는지만 센 표현 완화율이며 차단율을 나타내지 않는다.
답변의 사실성이나 근거 충실도를 측정한 값은 아니다. 근거를 조회하지 못했거나 답변
생성에 실패한 경우에는 이 규칙과 별도로 상태 배지를 붙인다.

## 답변 상태 배지

설계에는 검증 에이전트의 결과를 배지로 표시하는 안이 있었지만 에이전트는 아직
구현하지 않았다. 배지에 결과를 넣는 코드도 없어 모든 답에 "검증 미수행"이 붙었다.
이 상태로는 배지로 답변별 검증 여부를 구분할 수 없었다.
...
답변 생성 오류를 "근거가 부족합니다"로 표시하면 자료가 부족한 상황으로 오해할 수
있어 두 상태를 구분했다.
...
기존 질문에 계속 적절히 답하는지 확인한다. 픽스처에는 가상 데이터를 사용해
실데이터가 저장소에 들어오지 않도록 했다.
...
위쪽 Tools 차트는 도구 호출을 tool 런 타입으로 기록하지 않아 비어 있다.
따라서 이 차트로 도구 사용 여부를 확인할 수는 없다.
...
OWASP 목록과 대조하면서 대화 요청의 호출 한도가 빠져 있는 것을 확인했다.
로그인 잠금은 있었지만 로그인한 열람자가 대화를 반복 요청하는 경우에는 제한이 없었다.
...
토큰이나 모델 호출 비용의 전체 한도는 아니다. 시연과 운영 계정은 대개 관리자이므로
관리자에게도 같은 한도를 적용한다.

요청 횟수는 프로세스 안에서만 센다. 워커를 여럿 띄우면 한도가 워커 수만큼 늘어나므로 분산
환경 전체의 한도로 볼 수 없다.
