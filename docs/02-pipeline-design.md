...
정렬 도구 일곱 개를 같은 조건에서 돌렸다. 속도만 재면 안 된다는 것을 그때
알았다. 몇몇 도구는 인덱스를 만들 때 메모리가 모자라 멈췄다. 인덱싱과 정렬의
메모리 사용량이 서로 달랐기 때문이다. 속도와 정확도, 메모리를 함께 비교해 minibwa를
골랐다.
...
이 화면을 만들 때 네 가지를 조심했다.

단위를 줄마다 밝히고 막대를 쓰지 않는다. 리드·변이·행은 서로 다른 것을 센다.
한 변이가 전사체마다 여러 줄이 되므로 행 수와 변이 수가 다르다. 하나의 막대로
이으면 사실이 아닌 그림이 된다.

순서를 지어내지 않는다. 산출물은 파이프라인 차례로 들어오지 않는다. 실제 순서는
...
뺄셈으로 숫자를 만들지 않는다. 축에 걸린 수를 축별 합으로 세면 겹쳐서 두 번
세고, "전체 − 축 밖"으로 세도 맞지 않는다. 16,075 + 2,827 = 18,902 인데
해석 대상은 18,853이다. 한 변이의 어떤 행은 축에 걸리고 어떤 행은 안 걸려
49건이 양쪽에 다 있다. 리포트가 낸 합집합을 그대로 쓴다.
