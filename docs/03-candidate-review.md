...
이 방식을 뺀 이유는 다음 시료의 질환이 달라질 수 있기 때문이다. 특정 질환을 코드에
넣어 두면 그 질환과 가까운 후보가 위로 올라가고 해당 시료의 원인 후보는 아래로
밀린다. 실제 데이터에서도 확인했다. 미스센스·HIGH 3,589건 가운데 경로에 걸린
것은 219건이었다. 원인에 가까워서가 아니라 적혈구 관련 경로를 기준으로 삼아서
선택된 후보들이었다.

그래서 경로 대조와 경로 가중치, 유전자 패널 115개, 유전자별 근거 수준 하드코딩을
전부 뺐다.

[문진 수준의 임상 정보는 받을 수 있다](01-problem-and-context.md)는 것을 확인한
뒤에는 질환별 분석을 다시 넣었다. 다만 질환 정보는 코드가 아니라 입력으로 받는다.
...
문진이 없는 경우도 실제 운영 상태이다. 빈칸을 음성 결과로 해석하지 않고 질환 모델을
추측해 채우지도 않는다.
...
61.9%는 대조하지 못한 자리가 남아 있기 때문이다. **빈칸은 음성 결과가 아니다.** 못 잰 것과
재서 걸리지 않은 것을 같은 칸에 적지 않도록 계열마다 값과 못 잰 까닭을 함께 남긴다.
