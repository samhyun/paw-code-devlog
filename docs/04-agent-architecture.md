...
질문은 `direct` 경로로 분류해 분석 도구를 부르지 않고 바로 답한다.
...
모델은 레지스트리에 등록된 단계만 실행할 수 있으며 셸에는 접근할 수 없다.
...
목표 단계를 받으면 등록된 의존 관계에 따라 선행 단계부터 실행 순서를 만든다.
사용자나 모델이 순서를 지정할 필요가 없다.
...
병렬로 실행하며 짧은 머리말 생성에 드는 비용을 줄이려고 nano 모델을 쓴다.
사용자는 주 답변을 기다리는 동안 이 머리말을 먼저 받는다.
