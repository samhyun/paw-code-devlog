...
> 핵심: 평가 설계 · 하이브리드 검색 · 폴백 RAG

2층 판정에서 인용한 논문을 색인했다. 챗봇은 이 색인에서 근거를 찾아 답한다.
처음에는 낱말 검색(BM25)만 썼다. 색인 규모가 작았고, 임베딩 모델을 쓰면 새
선행 조건이 생겼기 때문이다. 당시에는 벡터 검색을 판단할 측정값이 없었다.
...
평가 방향을 다시 잡았다. 논문 40편을 무작위로 뽑아 제목만 모델에게 주고 "이 논문을
...
나가면 답할 것이 없었다. 그래서 확장 검색으로 74,876편을 더해 **77,531편**으로
키웠다.
...
세 모델을 같은 질의 40건으로 견줬다. 모두 77,531편 기준이다.
...
같은 한국어 질문에 무엇이 올라오는지가 이렇게 달라졌다.
...
경로는 "한국어 질문 → 모델이 영어 검색어 추출 → 검색"이다. 그 검색어를 열어 보니
유전자 이름을 지어내고 있었다.
...
문장 질의에서 적중률이 높았다. BM25 적중률도 함께 올라 두 검색 방식이 서로 빈틈을
채웠다.
...
⚠️ 검색이 됐다고 답이 맞는 것은 아니다. 논문은 찾았는데 챗봇이 엉뚱한 요약을
붙이면 틀린 말이다. 이 위험은 검색 방식으로 막을 수 없어서
[가드레일](06-ai-operations.md) 쪽에서 다룬다.
...
두기 위해서였다. 로컬 낱말 검색은 지우지 않았다. 아래 세 가지 역할은 로컬
검색에 남아 있다.
...
다음에는 실제 사용자가 던지는 질문으로 다시 재야 한다.
→ [확장 방향](08-next-steps.md)
